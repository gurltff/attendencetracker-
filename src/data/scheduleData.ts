...
const COURSE_KEY_SEP = '::'

/** Composite key identifying one specific subject taught within one programme + year,
 *  e.g. "B.Sc. (Hons) Physics::First Year::MP-I" — used for a teacher's assigned subjects
 *  so the same subject name in different years/programmes doesn't get mixed up. */
export function makeCourseKey(programName: string, year: string, course: string): string {
  return [programName, year, course].join(COURSE_KEY_SEP)
}
...
export function parseCourseKey(key: string): ParsedCourseKey {
  const [programName, year, course] = key.split(COURSE_KEY_SEP)
  return { programName, year, course }
}